...
private:
  ...
  const float K_angular = 10; //radians a second turn rate to avoid obstacles//1.0
  ...
  const int targetCountPivot = 10; ///unused variable
  const float obstacleDistancePivot = 0.3526; ///unused variable
  const float triggerDistance = .8;//.8
